...
const bacterialTypes = [
  { name: "MNE", fullName: "Meningite Não Especificada", value: 13, color: "hsl(0 72% 55%)" },
  { name: "MM", fullName: "Meningite Meningocócica", value: 3, color: "hsl(38 92% 50%)" },
  { name: "MM+MCC", fullName: "Meningo + Meningococcemia", value: 1, color: "hsl(210 80% 55%)" },
  { name: "MB", fullName: "Meningite por outras Bactérias", value: 1, color: "hsl(152 60% 45%)" },
];
    ...
    <div className="glass-card p-6">
        ...
        <div className="space-y-2">
          {etiologiaData.map((d) => (
            <div key={d.name} className="flex items-center gap-2 text-sm">
              ...
              <span className="text-xs text-muted-foreground">({((d.value / 21) * 100).toFixed(0)}%)</span>
            </div>
          ))}
        </div>
      </div>
          ...
          {bacterialTypes.map((b) => (
            <div key={b.name} className="flex items-center gap-2 p-2 bg-secondary/30 rounded-lg">
              ...
              <div className="min-w-0">
                <p className="text-xs font-semibold text-foreground">{b.name} — {b.value}</p>
      ...
      <div className="mt-4 p-3 rounded-lg bg-destructive/10 border border-destructive/20">
        <p className="text-xs text-destructive">
          <span className="font-semibold">Análise:</span> A MNE é predominante com 13 casos (72% das bacterianas), seguida por MM com 3 casos (17%). A etiologia bacteriana domina com 86% do total.
        </p>
      </div>
    </div>
  );
